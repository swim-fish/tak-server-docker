...
1. 核對目前容器、註冊名稱及驗證資料的指紋，避免操作到選取後已變更的身分。清單由按需啟動的 `mumble-db-helper` 經 `mumble-data` volume 唯讀取得，不使用 `docker exec`。
2. 同一輔助容器使用 SQLite 一致性備份保留完整 Mumble 資料庫到 `runtime/mumble-admin/`，並驗證 `integrity_check`；失敗時停止操作。輔助容器沒有網路，備份目錄保留在 Windows 主機。
