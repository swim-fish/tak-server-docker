...
「依群組檢視」子頁以群組為主，分別列出 In／寫入、Out／讀取及 In + Out／讀寫的憑證，並連到各憑證詳細頁。憑證清冊與群組子頁都預設只看「使用中」；群組子頁可按「全選／使用中／逾期／撤銷」切換，頁尾的「未記錄群組」套用相同篩選。每張憑證以 Badge 標示狀態。該頁使用憑證管理程式保存的群組登錄紀錄；若群組在控制台以外變更，應進入憑證詳細頁向 TAK API 核對。

在群組子頁，可將使用中且已註冊的憑證拖到群組的 In、Out 或 In + Out 清單；觸控裝置可按憑證旁的「移動」選擇目的群組與權限。同群組拖曳會改變 In／Out 權限；拖往另一群組會移除來源群組的權限，並設定目的群組權限。「新增憑證」Popup 只列出尚未加入該群組的使用中、已註冊憑證，提供搜尋、勾選及目的權限選擇。所有變更先暫存在瀏覽器，按「儲存群組變更」並勾選確認後才送出；主機程式先從 TAK API 核對原群組，若資料已變動就拒絕這批更新，避免覆蓋其他操作。更新後逐張讀回，部分失敗會顯示各張憑證結果。
...
在控制台選 **引導式佈建 → 新增 TAK 用戶端**，可加入 1 至 10 台裝置。每台分別填顯示名稱、唯一 CN，並使用獨立的拖曳式群組清單設定權限。寬螢幕並排「未指派」與 In／寫入、Out／讀取、In + Out／讀寫四欄；窄螢幕收成兩欄或單欄。可多選後按「移動」，也可輸入新群組名稱，先加入未指派清單，再移到所需權限；每台至少要有一個 In 或 Out 群組。預覽頁列出每台設定與 QR 分享期限、下載上限；確認後才簽發。每張憑證、私鑰、PKCS#12 與 DPK 獨立建立，註冊資訊合併寫入 `UserAuthenticationFile.xml`，整批只重啟 TAK Server 一次，接著逐張以管理 API 讀回群組。
...
2026-09-24 已在本機用兩張測試憑證驗證合併註冊、單次重啟、兩筆 API 群組讀回及相同作業 ID 重送不重新簽發，見[引導式佈建驗證](../validation/2026-09-24-guided-provisioning.md)。裝置端透過這批新 DPK 的匯入與登入仍待實測。
