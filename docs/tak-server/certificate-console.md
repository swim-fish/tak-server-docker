...
在群組子頁，可將使用中且已註冊的憑證拖到群組的 In、Out 或 In + Out 清單；觸控裝置可按憑證旁的「移動」選擇目的群組與權限。同群組拖曳會改變 In／Out 權限；拖往另一群組會移除來源群組的權限，並設定目的群組權限。按「從此群組移除」或拖到「未記錄群組」，可移除來源群組的 In／Out 權限；若這是最後一個群組，憑證會變成未指派。TAK 5.8 管理 API 已用測試憑證驗證可清空群組並還原。「新增憑證」Popup 只列出尚未加入該群組的使用中、已註冊憑證，提供搜尋、勾選及目的權限選擇。所有變更先暫存在瀏覽器，按「儲存群組變更」並勾選確認後才送出；主機程式先從 TAK API 核對原群組，若資料已變動就拒絕這批更新，避免覆蓋其他操作。更新後逐張讀回，部分失敗會顯示各張憑證結果。
