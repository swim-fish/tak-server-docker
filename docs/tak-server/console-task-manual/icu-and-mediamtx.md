...
2. 自訂時選 Advanced，具名小隊的 Stream Path 必須留在 `live/<小隊>/` 下並以 `/` 結尾。ICU 會在末端加上 `VIDEO_1`。
3. 設定 QR 時間與下載上限，預覽後交付完整 `icu://download?url=...`。
...
3. 按目的選「再次發布 ICU QR」、「停用選取身分」或「重設選取密碼」，勾選頁面確認後送出。
