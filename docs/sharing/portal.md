...
這些操作由 Windows 前景程式執行，Flask 容器沒有 Docker socket。另開一個一般 PowerShell 視窗，執行：

```powershell
python .\scripts\mumble_control_host.py
```

保持視窗開啟才能使用 Mumble 管理；按 Ctrl+C 只停止該管理程式，不停止 Mumble 或分享服務。刪除註冊與重設密碼需在確認視窗勾選，確認按鈕才會啟用。操作前若有人在線，先通知相關使用者；此介面的中斷與重啟會立即影響連線。備份留在版控忽略的 `runtime/mumble-admin/` 與 `runtime/share-control/backups/`，均不可公開。Vx 無法在 UI 指定任意 Mumble Username，詳見 [Mumble 使用者](../mumble/users.md)。
...
健康檢查只代表 Web 服務存活。要驗證 QR，先建立短效、單次分享，再從熱點裝置掃碼並核對實際 ICU 欄位或 ATAK 匯入結果。完成後停止分享、防火牆前景工作階段與 Mumble 管理程式；若要關閉 Web 服務，執行 `docker compose --profile sharing stop share-admin share-public`。
