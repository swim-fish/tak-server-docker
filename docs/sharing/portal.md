...
分享紀錄使用辨識名稱，不更改實際下載檔名：標準 ICU 顯示 `ICU-<小隊>-<隊員編號>`，未指定隊員時顯示「未指定」；Advanced ICU 顯示 `ICU-ADV-<path>`；TAK 憑證 DPK 顯示 `<CN>-<CRL ID>`。舊紀錄若缺少建立時的中繼資料，會盡量從路徑或套件檔名推導；無法辨識的小隊資訊顯示「ICU-預設」。
