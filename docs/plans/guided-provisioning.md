...
管理者可從清冊選一張或多張**有效、已註冊且有對應 DPK** 的既有憑證，各自建立分享；已撤銷、過期或註冊未驗證者不得產生可用 QR。另一入口可新增單筆或批次憑證，每筆填裝置顯示名稱、唯一 CN、憑證到期日期時間、In／Out 群組；批次可逐筆調整。到期欄位是台灣時間的日曆與時間選擇器，留空採兩年預設，可設 5 分鐘至 730 天的短效憑證，但不得超過中繼 CA 效期。預覽列出每筆的 CN、到期時間、群組及預期產物。
